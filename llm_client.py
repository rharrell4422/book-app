...
def call_llm(
    ...
    *,
    ...
    SDK error, unrecognized model_id) -- never returns a partial/None
    response. Callers that need "record telemetry even on failure, with
    zero tokens" (matching today's behavior at all 3 existing call sites)
    should catch `LLMCallError` and record their own zero-token entry,
    the same way they previously did with `response` staying `None`.
    """
    provider = _provider_for_model(model_id)
    if provider == "anthropic":
        return _call_anthropic(model_id, prompt, max_tokens=max_tokens, temperature=temperature, timeout=timeout)
    raise LLMCallError(f"no dispatch implemented for provider={provider!r} (model_id={model_id!r})")
