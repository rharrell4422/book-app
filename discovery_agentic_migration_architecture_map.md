...
- Identity/dedupe keys used for matching a discovered candidate against existing rows (in priority order): ASIN exact match → `_series_book_identity_key(series_name, book_number)` → `_canonical_title_identity_key(title)` (all from `services/identity.py`) — three separate collapse passes run after insert to catch cross-key duplicates the initial matching missed (lines 642-738).
