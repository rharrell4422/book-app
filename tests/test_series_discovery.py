...
class SeriesDiscoveryRegressionTest(unittest.TestCase):
    ...
    def test_series_suggest_endpoint_includes_publication_date_filter_shape(self):
        ...
        with patch("main.series_agent.discover", return_value=mocked_payload):
            response = client.get(f"/series/{self.series.id}/suggest", params={"book_number": 7})
        ...
        self.assertIn("publication_date_filter", payload["diagnostics"])
        publication_filter = payload["diagnostics"]["publication_date_filter"]
        for key in ["series_complete", "date_future", "date_missing", "accepted", "reason"]:
            self.assertIn(key, publication_filter)
        self.assertIsInstance(publication_filter["series_complete"], bool)
        self.assertIsInstance(publication_filter["date_future"], bool)
        self.assertIsInstance(publication_filter["date_missing"], bool)
        self.assertIsInstance(publication_filter["accepted"], bool)
        self.assertIsInstance(publication_filter["reason"], str)

    def test_books_suggest_endpoint_uses_series_context_for_discover(self):
        ...
        with patch("main.crud.get_series_by_name", return_value=SimpleNamespace(total_books=9, is_finished=True, series_status="completed", author="Harmon Cooper")):
            with patch("main.series_agent.discover", return_value=mocked_payload) as mock_discover:
                response = client.get(
                    "/books/suggest",
                    params={
                        "series_name": "Cherry Blossom Girls",
